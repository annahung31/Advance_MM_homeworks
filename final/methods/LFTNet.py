...
class LFTNet(nn.Module):
  def __init__(self, params, tf_path=None, change_way=True):
    ...
    if params.method == 'protonet':
      ...
      model = myMethod.MyMethod( model_dict[params.model], tf_path=params.tf_dir, **train_few_shot_params, ptl=True)
  ...
  def trainall_loop(self, epoch, ps_loader, pu_loader, aux_iter, total_it):
    ...
    for i, ((x, _), (x_new, _)) in enumerate(zip(ps_loader, pu_loader)):
      ...
      model_loss = self.model.set_forward_loss(x)
      ...
      self.model.eval()
      ft_loss = self.model.set_forward_loss(x_new)
