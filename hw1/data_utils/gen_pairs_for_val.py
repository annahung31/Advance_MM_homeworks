...
class GeneratePairs:
    ...
    def _generate_mismatches_pairs(self):
        ...
        for i, name in enumerate(os.listdir(self.data_dir)):
            ...
            waited_list1 = os.listdir(self.data_dir + name)
            waited_list1 = [f_n for f_n in waited_list1 if f_n.split('.')[-1] == 'jpg']

            waited_list2 = os.listdir(self.data_dir + other_dir)
            waited_list2 = [f_n for f_n in waited_list2 if f_n.split('.')[-1] == 'jpg']

            with open(self.pairs_filepath, "a") as f: 
                for i in range(3):
                    file1 = random.choice(waited_list1)
                    # print('first', file1, name)
                    file2 = random.choice(waited_list2)
                    # print('second', file2, other_dir)
                    number_1 = file1.split('_')[-1].rstrip(self.img_ext)
                    number_2 = file2.split('_')[-1].rstrip(self.img_ext)
