    ...
    def write_item_label(self):
        ...
        for name in tqdm(os.listdir(self.data_dir)):
            ...
            for file in os.listdir(self.data_dir + '/' + name):
                ...
                bbox = np.load(file_path + '_bbox.npy')
                lnmk = np.load(file_path + '_lnmk.npy')
                
                data = str(1) + '\t' + file_path + '\t' + str(cnt) + '\t' + \
                        str(bbox[0]) + '\t' + str(bbox[1]) + '\t' + str(bbox[2]) + '\t' + str(bbox[3]) + '\t' + \
                        str(lnmk[0][0]) + '\t' + str(lnmk[0][1]) + '\t' + \
                        str(lnmk[1][0]) + '\t' + str(lnmk[1][1]) + '\t' + \
                        str(lnmk[2][0]) + '\t' + str(lnmk[2][1]) + '\t' + \
                        str(lnmk[3][0]) + '\t' + str(lnmk[3][1]) + '\t' + \
                        str(lnmk[4][0]) + '\t' + str(lnmk[4][1]) + '\t' + '\n'
...
if __name__ == '__main__':
    ...
    # python insightface_pairs_gen_v1.py --dataset-dir ../train/tr_split/ --list-file train.lst --img-ext jpg
    ...
    parser.add_argument('--dataset-dir', default='/home/annahung/189/courses/AMMAI/hw1/data/train/tr_split', help='Full path to the directory with peeople and their names, folder should denote the Name_Surname of the person')
    parser.add_argument('--list-file', default='train.lst', help='Full path to the directory with peeople and their names, folder should denote the Name_Surname of the person')
