...
from dataset import APDDataset, DataLoaderX
...
'''
python -m torch.distributed.launch --nproc_per_node=1 --nnodes=1 --node_rank=0 --master_addr=140.109.135.76 --master_port=1234 train.py
'''
myseed = 42069 
...
def main(args):
    pretrain_model_path = 'models/0412-0642/backbone_2232.pth'
    ...
    trainset = APDDataset(root_dir=cfg.rec, local_rank=local_rank)
    ...
    if args.resume:
        try:
            backbone_pth = pretrain_model_path
    ...
    callback_verification = CallBackVerification(1000, rank, cfg.val_targets, cfg.rec)
